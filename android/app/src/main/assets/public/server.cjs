...
var import_googlethis = __toESM(require("googlethis"), 1);
var import_youtube_search_api = __toESM(require("youtube-search-api"), 1);
...
async function startServer() {
  ...
  app.get("/api/transcript", async (req, res) => {
    ...
    try {
      const transcript = await import_youtube_transcript.YoutubeTranscript.fetchTranscript(videoId);
      res.json(transcript);
    } catch (e) {
  ...
  app.get("/api/search_suggestions", async (req, res) => {
    ...
    try {
      const response = await import_axios.default.get(`https://duckduckgo.com/ac/?q=${encodeURIComponent(q)}`);
      res.json(response.data);
    } catch {
      res.json([]);
    }
  });
  const instanceBlacklist = /* @__PURE__ */ new Map();
  const BLACKLIST_DURATION = 1e3 * 60 * 30;
  const instances = [
  ...
  app.get("/api/search", async (req, res) => {
    ...
    if (TINYFISH_API_KEY) {
      try {
        ...
        for (const endpoint of tfEndpoints) {
          try {
            tfResponse = await import_axios.default.post(endpoint, {
              query: q,
              ...
              timeout: 4e3
              // Fast fail for endpoint discovery
            });
        ...
        if (tfResponse && tfResponse.data && tfResponse.data.results && Array.isArray(tfResponse.data.results)) {
          const results = tfResponse.data.results.map((r) => ({
    ...
    for (const searxUrl of shuffledInstances) {
      try {
        ...
        const response = await import_axios.default.get(searxUrl, {
          headers: {
            "User-Agent": randomUA,
    ...
    if (category === "images") {
      try {
        const googleImages = await import_googlethis.default.image(q, { safe: false });
        if (googleImages && googleImages.length > 0) {
          const results = googleImages.slice(0, 20).map((r) => ({
      ...
      try {
        const wikiSearch = await import_axios.default.get(`https://en.wikipedia.org/w/api.php?action=query&generator=search&gsrsearch=${encodeURIComponent(q)}&gsrlimit=20&prop=pageimages&piprop=original|thumbnail&pithumbsize=600&format=json`, { headers: fallbackHeaders });
        if (wikiSearch.data?.query?.pages) {
          const results = Object.values(wikiSearch.data.query.pages).filter((p) => p.thumbnail || p.original).map((p) => {
      ...
      try {
        const ytResults = await import_youtube_search_api.default.GetListByKeyword(q, false, 15, [{ type: "video" }]);
        if (ytResults && ytResults.items && ytResults.items.length > 0) {
          const results = ytResults.items.filter((r) => r.type === "video").map((r) => ({
            title: r.title,
      ...
      try {
        const googleSearch = await import_googlethis.default.search(`site:twitter.com OR site:instagram.com OR site:reddit.com OR site:linkedin.com OR site:youtube.com OR site:facebook.com OR site:tiktok.com ${q}`, { page: 0, safe: false, parse_ads: false });
        if (googleSearch && googleSearch.results && googleSearch.results.length > 0) {
          const results = googleSearch.results.map((r) => ({
            title: r.title,
            url: r.url,
      ...
      try {
        const googleSearch = await import_googlethis.default.search(q, { page: 0, safe: false, parse_ads: false });
        if (googleSearch && googleSearch.results && googleSearch.results.length > 0) {
          const results = googleSearch.results.map((r) => ({
            title: r.title,
            url: r.url,
      ...
      try {
        const wikiSearch = await import_axios.default.get(`https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch=${encodeURIComponent(q)}&utf8=&format=json`, { headers: fallbackHeaders });
        if (wikiSearch.data?.query?.search) {
          const results = wikiSearch.data.query.search.map((r) => ({
            title: r.title,
  ...
  app.get("/api/articles/trending", async (req, res) => {
    ...
    try {
      ...
      const lcResponse = await import_axios.default.post(LONGCAT_BASE_URL, {
        model: "LongCat-Flash-Chat",
        // Switching to Chat for better reliability
        messages,
        stream: false,
        max_tokens: 4e3,
        temperature: 0.3
      ...
      $(".category-section").each((_, catElem) => {
        ...
        const papers = [];
        $(catElem).find("article").each((_2, artElem) => {
  ...
  function mapToOpenAI(prompt, history, systemInstruction) {
    const messages = [];
    ...
    if (history) {
      for (const msg of history) {
        ...
        const textStr = Array.isArray(msg.parts) ? msg.parts.map((p) => p.text).join("\n") : msg.content || msg.text || "";
        if (textStr) {
  ...
  app.post("/api/gemini/generate", async (req, res) => {
    ...
    try {
      ...
      const response = await import_axios.default.post(LONGCAT_BASE_URL, {
        model: targetModel,
        messages,
        stream: false,
